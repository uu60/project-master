...
public class AccountController {
    ...
    public R register(@RequestBody UsernamePasswordVO vo) {
        ...
        if (accountEntity != null) {
            return R.exception(R.Codes.USERNAME_EXISTED, "Username already exists.");
        }
        try {
            boolean res = accountService.addAccount(vo);
            return res ? R.ok() : R.exception(R.Codes.USERNAME_OR_PASSWORD_WRONG_FORMAT, "Invalid username or password.");
        } catch (Exception e) {
            e.printStackTrace();
            return R.exception(R.Codes.SERVER_INTERNAL_ERROR, "Server internal error.");
        }
    }
}
